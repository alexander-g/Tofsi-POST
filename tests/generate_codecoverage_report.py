...
LOG_DIR           = 'latest_logs'
INPUT_DIR         = f'{LOG_DIR}/codecoverage/raw'
OUTPUT_DIR        = f'{LOG_DIR}/codecoverage/reports'
...
for js in jsonfiles:
    ...
    jsondata = dict([ (x['url'], x) for x in jsondata if 'thirdparty' not in x['url']])
...
for url, ranges in merged_ranges.items():
    ...
    basename    = os.path.basename(url) or 'index.html'
